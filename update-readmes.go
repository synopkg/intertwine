...
// the github.com/synopkg/intertwine tree. It only updates files if they are
...
func main() {
	root, err := build.Import("github.com/synopkg/intertwine", "", build.FindOnly)
	if err != nil {
		log.Fatalf("failed to find github.com/synopkg/intertwine root: %v", err)
	}
	err = filepath.Walk(root.Dir, func(path string, fi os.FileInfo, err error) error {
		...
		pkgName := "github.com/synopkg/intertwine/" + filepath.ToSlash(rest)
